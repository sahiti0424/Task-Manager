...
const Schedule = ({ tasks = [], schedule = mockSchedule, subjects = mockSubjects }) => {
  ...
  return (
    <div className="p-3 md:p-6 space-y-6 max-w-7xl">
      <Section delay={0.1}>
        <div
          className="flex items-center justify-between bg-white border border-amber-200 rounded-lg px-3 md:px-5 py-2.5 md:py-3.5"
          style={cardStyle}
        >
          <button
            onClick={() => setWeekOffset((w) => w - 1)}
            className="w-7 md:w-8 h-7 md:h-8 rounded-md hover:bg-amber-100 flex items-center justify-center text-amber-600"
          >
            ‹
          </button>

          <div className="text-center flex-1">
            <p className="text-xs md:text-sm font-semibold text-stone-900">
        ...
        >
          <div className="overflow-x-auto">
            {HOURS.map((hour) => (
              <div
                key={hour}
                className="grid border-b border-amber-50 min-h-12"
                style={{
                  gridTemplateColumns: 'auto repeat(7, minmax(60px, 1fr))'
                }}
              >
                <div className="p-2 border-r border-amber-100 bg-amber-50/50 flex items-start w-12 md:w-16">
                  <span className="text-[8px] md:text-[10px] text-amber-500 font-medium">
                    {hour % 12 || 12}
                    {hour < 12 ? 'a' : 'p'}
                  </span>
                </div>
                ...
                {DAYS.map((day, di) => {
                  ...
                  return (
                    <div
                      key={day}
                      className={`border-r border-amber-50 last:border-0 p-1 ${
                    ...
                    >
                      {slots.map((slot) => {
                        ...
                        return (
                          ...
                          >
                            <p className="text-[8px] md:text-[10px] font-bold truncate">
                              {sub?.code || 'Class'}
                            </p>

                            <p className="text-[7px] md:text-[8px] text-amber-300">
                      ...
                      {deadlines.map((t) => (
                        <div
                          key={t.id}
                          className="rounded-md p-1 mb-1 bg-white border border-amber-200 shadow-sm"
                        >
      ...
      {schedule.length > 0 && (
        <Section delay={0.3}>
          <div
            className="bg-white rounded-lg border border-amber-200 p-4 md:p-6"
            style={cardStyle}
          >
